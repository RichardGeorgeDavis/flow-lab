...
class mlsMpmSimulator {
    ...
    async init() {
        ...
        this.uniforms.hueShift = uniform(0);
        this.uniforms.hueRange = uniform(0);
        this.uniforms.hueDrift = uniform(0);
        this.uniforms.saturationBase = uniform(0);
        this.uniforms.saturationVelocity = uniform(0);
        this.uniforms.valueBase = uniform(0);
        this.uniforms.valueForce = uniform(0);
        ...
        this.kernels.g2p = Fn(() => {
            ...
            const hue = particleDensity.div(this.uniforms.restDensity).mul(this.uniforms.hueRange).add(this.uniforms.hueShift).add(time.mul(this.uniforms.hueDrift));
            const saturation = particleVelocity.length().mul(0.5).clamp(0,1).mul(this.uniforms.saturationVelocity).add(this.uniforms.saturationBase).clamp(0, 1);
            const value = force.mul(this.uniforms.valueForce).add(this.uniforms.valueBase).clamp(0, 1);
    ...
    async update(interval, elapsed) {
        const { particles, run, noise, dynamicViscosity, stiffness, restDensity, speed, gravity, gravitySensorReading, accelerometerReading, hueShift, hueRange, hueDrift, saturationBase, saturationVelocity, valueBase, valueForce } = conf;
        ...
        this.uniforms.hueShift.value = hueShift;
        this.uniforms.hueRange.value = hueRange;
        this.uniforms.hueDrift.value = hueDrift;
        this.uniforms.saturationBase.value = saturationBase;
        this.uniforms.saturationVelocity.value = saturationVelocity;
        this.uniforms.valueBase.value = valueBase;
        this.uniforms.valueForce.value = valueForce;
